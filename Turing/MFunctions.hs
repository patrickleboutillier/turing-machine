module Turing.MFunctions (f, g, g'2, e'1, e'2, e'3, pe, l, r, f', f'', c, ce'2, ce'3, 
...
-- page 115
f :: Domain -> (MConfig, MConfig, Symbol) -> MConfig
f  d x@(mC, mB, a) = mF "f"  x      [Sym "e"        [L]         $ f1 d (mC, mB, a),
                                     Not "e"        [L]         $ f  d (mC, mB, a),
                                     None           [L]         $ f  d (mC, mB, a)]
f1 d x@(mC, mB, a) = mF "f1" x      [Sym a          []          $ mC,
                                     Not a          [R]         $ f1 d (mC, mB, a),
                                     None           [R]         $ f2 d (mC, mB, a)]
f2 d x@(mC, mB, a) = mF "f2" x      [Sym a          []          $ mC,
                                     Not a          [R]         $ f1 d (mC, mB, a),
                                     None           [R]         $ mB]


-- page 118
e'3  d x@(mC, mB, a) = mF "e'3"  x  [Blank          []          $ f d (e1'3 d (mC, mB, a), mB, a)]
e1'3 d x@(mC, mB, a) = mF "e1'3" x  [Blank          [E]         $ mC]

e'2  d x@(mB, a)     = mF "e'2"  x  [Any            []          $ e'3 d (e'2 d (mB, a), mB, a)]


-- page 120
pe  d x@(mC, b)     = mF "pe"  x    [Blank          []          $ f d (pe1 d (mC, b), mC, "e")]
pe1 d x@(mC, b)     = mF "pe1" x    [Any            [R, R]      $ pe1 d (mC, b),
                                     None           [P b]       $ mC]


-- page 121
l d x@(mC) = mF "l" x               [Blank          [L]         $ mC]
r d x@(mC) = mF "r" x               [Blank          [R]         $ mC]

f'  d x@(mC, mB, a) = mF "f'"  x    [Blank          []          $ f d (l d mC, mB, a)] 
f'' d x@(mC, mB, a) = mF "f''" x    [Blank          []          $ f d (r d mC, mB, a)] 

c  d x@(mC, mB, a) = mF "c"  x      [Blank          []          $ f' d (c1 d mC, mB, a)]
c1 d x@(mC)        = mF "c1" x  
                        $ map (\b -> Blank          []          $ pe d (mC, b)) d 
